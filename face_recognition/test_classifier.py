...
box_coords = dict()
with open('./output/coords.txt', 'r') as file:
    r = str(file.read())
box_coords = json.loads(r)
...
x
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        img_dataset = _get_test_and_train_set(input_directory, min_num_images_per_label=min_images_per_labels,
        ...
        images, labels, class_names = _load_images_and_labels(img_dataset, image_size=160, batch_size=batch_size,
...
def _evaluate_classifier(emb_array, label_array, classifier_filename):
    logger.info('Testing classifier on {} images'.format(len(emb_array)))
    ...
    with open(classifier_filename, 'rb') as f:
        ...
        for i in range(len(best_class_indices)):
            if(best_class_probabilities[i] >= 0.5):
                #print('%s  %s: %.3f' % (imgs_name_list[i], class_names[best_class_indices[i]], best_class_probabilities[i]))
                tagged_img_name = './tagged/' + dirname +'/' + str(imgs_name_list[i]).split('_')[0] + '.jpg'
                tagged_img = cv2.imread(tagged_img_name)
                coords = box_coords[imgs_name_list[i]]
                prob = int(best_class_probabilities[i] * 100)
                cv2.putText(img=tagged_img,text=class_names[best_class_indices[i]] + ': ' + str(prob) +'%',org=(coords[0],coords[1]),color=(0,255,0),fontFace=cv2.FONT_HERSHEY_PLAIN,fontScale=1)
                cv2.imwrite(tagged_img_name,tagged_img)
...
if __name__ == '__main__':
    ...
    folder = os.path.join(args.input_dir,os.listdir(args.input_dir)[0])
    dirname = os.listdir(args.input_dir)[0]
    #print(dirname)
    imgs_name_list = os.listdir(folder)
